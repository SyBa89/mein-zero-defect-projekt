...
export default function MobileActionBar() {
  ...
  // ✅ ZERO-DEFECT: Fallback für außerhalb des 🗺️ Router-Kontexts
  const isContactActive = useMemo(() => {
    if (!pathname) return false;
    return pathname === '/✉️ Kontakt';
  }, [pathname]);
    ...
    >
      <div className="bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] pb-[env(safe-area-inset-bottom)]">
        <div className="grid grid-cols-3 h-16">
          ...
          {/* 2. 🗺️ Route planen (Google Maps Deep Link) */}
          <a
            href={MAPS_LINK}
            target="_blank"
            ...
            aria-label={`🗺️ Route zu ${KIOSK_CONFIG.name} mit Google Maps planen`}
          >
            <svg
              className="w-6 h-6"
              fill="none"
            ...
            <span className="text-xs font-bold">🗺️ Route</span>
          </a>

          {/* 3. ✉️ Kontaktformular (Mit aktiver Status-Erkennung) */}
          <Link
            href="/✉️ Kontakt"
            ...
            aria-label="✉️ Kontaktformular öffnen"
          ...
          >
            ...
            <span className="text-xs font-bold">✉️ Kontakt</span>
          </Link>
        </div>
